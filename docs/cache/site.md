...
- Looping motion (hero Ken Burns, hummingbird) pauses off-screen. Client logos use a CSS `translate3d` marquee (two copies, no GSAP ticker/Draggable, no page-scroll listener). Arabic reverses the loop. It pauses on hover and when off-screen. Logos are plain `img` tags (`loading="lazy"`, `decoding="async"`). Nav scroll listeners are rAF-throttled and do not use `backdrop-filter`.
