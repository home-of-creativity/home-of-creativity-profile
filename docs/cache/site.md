...
- Looping motion (hero Ken Burns, hummingbird) pauses off-screen. Client logos use a CSS `translate3d` marquee (two copies, no GSAP ticker/Draggable); it pauses while the page is scrolling and when off-screen. Nav scroll listeners are rAF-throttled and do not use `backdrop-filter`.
